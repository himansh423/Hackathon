...
export default function Chatbot() {
  ...
  // Function to fetch crawled website content
  ...
  const chat = async () => {
    try {
      setLoading(true);
      let webContent = crawledData;

      // Fetch website content only if not already fetched
      if (url && !webContent) {
        webContent = await crawlWebsite(url);
        setCrawledData(webContent); // Store the crawled data
      }

      // Generate prompt
      const prompt = `
        Context from webpage: ${webContent.slice(0, 5000)}  // Limit text to avoid API overflow
